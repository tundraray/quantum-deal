...
WHERE created_at >= date_trunc('month', CURRENT_DATE)
  AND closed_at IS NOT NULL  -- Only closed deals
;
...
COMMENT ON MATERIALIZED VIEW monthly_bot_statistics IS
  'Cached monthly statistics for bot onboarding. Refreshed every 15 minutes via cron job. Only includes closed deals (closed_at IS NOT NULL).';
