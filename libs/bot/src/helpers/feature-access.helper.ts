...
export function hasAnyFeatureAccess(
  ...
  const user = ctx.user;
  return features.some((feature) => hasFeature(user, feature));
}

/**
 * Check if user has all of the specified features
 *
 * Returns true only if user has all features.
 *
 * @param ctx - Telegraf context with user
 * @param features - Array of features to check
...
export function hasAllFeaturesAccess(
  ...
  const user = ctx.user;
  return features.every((feature) => hasFeature(user, feature));
}

/**
 * Assert user has feature or throw error
 *
 * Throws an error with a user-friendly message if user doesn't have the feature.
 * Use this when you want to halt execution immediately.
 *
 * @param ctx - Telegraf context with user
 * @param feature - Required feature
 * @throws Error if user doesn't have the feature
 */
export function requireFeatureOrThrow(
  ctx: UserContext,
  feature: FeatureFlag,
): void {
  if (!checkFeatureAccess(ctx, feature)) {
    throw new Error(`Feature ${feature} is required but not available`);
  }
}
